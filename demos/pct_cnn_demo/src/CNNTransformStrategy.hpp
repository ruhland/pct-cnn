...
private:
	...
	void createNormals(typename PointCloud<PointT>::Ptr cloud,
		...
		for (int i = 0; i < normals->size(); i++) {
			if (!pcl::isFinite<pcl::Normal>((*normals)[i])) {
				cerr << "[CNNTransformStrategy::createNormals]" << " Normal "
						<< i << " " << (*normals)[i] << " is not finite"
						<< endl;
			}
		}
	}
	...
	void computePFHFeatures(PointCloud<PointXYZRGB>::Ptr &points,
		...
		cout << "[CNNTransformStrategy::computePFHFeatures]"
				<< " Setting input cloud: " << points->size() << " points"
				<< endl;
		cout << "[CNNTransformStrategy::computePFHFeatures]"
	...
	void findCoherentNeighbours(
			typename PointCloud<FeatureT>::Ptr source_features,
		...
		for (size_t i = 0; i < source_features->size(); i++) {
			searchtree.nearestKSearch(*source_features, i, k, k_indices,
					k_squared_distances);
			for (int y = 0; y < k; y++) {
	 ...
	 * inverted.
	 */
	void findPerfectCoherentNeighbours(
			PointCloud<PointXYZRGB>::Ptr source_features,
	...
	void replaceColors(PointCloud<PointXYZRGB>::Ptr &src,
		...
		for (int i = 0; i < src->size(); i++) {
			...
			std::sort(coherent_neighbors[i].begin(),
					coherent_neighbors[i].end());
	...
	float getMeanSquaredFaceError(
			vector<vector<PointWithScore>> &coherentNeighbors) {
		float error = 0;
...
public:
	typename PointCloud<PointT>::Ptr transform(
		...
		typename PointCloud<PointXYZRGB>::Ptr result(
		...
		for (float xdegree = 0; xdegree < 360.0f; xdegree+=configuration->getInt("degreesteps", 5)) {
			...
			case PFHFEATURE:
				...
				computePFHFeatures(targetFiltered, target_normals,
						target_descriptors,
						configuration->getFloat("pfhradius"));
				findCoherentNeighbours<PFHSignature125>(source_descriptors,
						target_descriptors, coherentNeighboursSource, k);
				findCoherentNeighbours<PFHSignature125>(target_descriptors,
