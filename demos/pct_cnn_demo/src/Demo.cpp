...
Demo::Demo() :
		visualizer(), kinect(), targetCloud(
				new pcl::PointCloud<pcl::PointXYZRGB>), sourceCloud(
				new pcl::PointCloud<pcl::PointXYZRGB>), configuration(
				new Configuration()) {
...
void Demo::setTargetFile(const std::string& name) {
	...
	//For Testing
	DemoVisualizer::rotateXYZ(targetCloud,
			configuration->getFloat("targetrotatex", 0.0f),
			configuration->getFloat("targetrotatey", 0.0f),
			configuration->getFloat("targetrotatez", 0.0f));
	visualizer.setTargetPC(targetCloud);
}
void Demo::setConfigFile(const std::string& file) {
	configuration->loadFromFile(file);
	std::cout << "Configuration read: normalradius: "
			<< configuration->getFloat("normalradius") << std::endl;
}
...
void Demo::run() {
	if (configuration->getBool("screenshotmode")) {
		visualizer.screenshot(transformer->transform(sourceCloud, targetCloud,
				configuration));
	} else {
		int lastface = 0;
		visualizer.show();
		...
		while (!visualizer.wasStopped()) {
			...
			if (lastrequestedTransofmation
					< visualizer.getRequestedTransformations()) {
				lastrequestedTransofmation =
						visualizer.getRequestedTransformations();
				visualizer.setTransformedPC(
						transformer->transform(sourceCloud, targetCloud,
								configuration));
			}
		}
		kinect.disconnect();
...
int main(int argc, char **argv) {
	try {
		...
		desc.add_options()("help", "produce help message")("t",
				...
				"target file (required)")("s",
				boost::program_options::value<std::string>(), "source file")(
				"f", boost::program_options::value<std::string>(),
				"file for parameter configuration")("k",
				"enable Kinect instead of source file");
